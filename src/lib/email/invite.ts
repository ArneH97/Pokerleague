...
function copy(i: InviteMailInput): Copy {
  ...
  if (i.locale === 'fr') {
    return {
      ...
      platform: `${clubName} les tient à jour sur PokerLeague, la plateforme où les clubs belges gèrent leurs tournois. Finalisez votre compte et vous les verrez vous-même, sur votre téléphone, après chaque soirée. Si vous jouez plus tard dans un autre club sur PokerLeague, tout se retrouve au même endroit.`,
  ...
  if (i.locale === 'en') {
    return {
      ...
      platform: `${clubName} keeps them on PokerLeague, the platform Belgian clubs use to run their tournaments. Finish your account and you can see them yourself, on your phone, after every night. Play at another club on PokerLeague later and it all lands in the same place.`,
  ...
  return {
    ...
    platform: `${clubName} houdt die bij op PokerLeague, het platform waar Belgische clubs hun tornooien draaien. Maak je account af en je ziet ze zelf, op je gsm, na elke avond. Speel je later bij een andere club op PokerLeague, dan komt dat er gewoon bij.`,
    cta: 'Account afmaken',
